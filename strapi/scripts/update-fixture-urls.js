...
const BACKUP_PATH = path.join(__dirname, '..', 'data', 'data.json.backup');
...
async function updateFixtureUrls() {
  ...
  await fs.copy(DATA_PATH, BACKUP_PATH);
  ...
  console.log(`\n✓ Updated data written to: ${DATA_PATH}`);
  ...
  console.log('\nNext step:');
  console.log('  npm run seed:example (to test with updated URLs)');
}
