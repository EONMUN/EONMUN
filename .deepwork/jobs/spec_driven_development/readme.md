...
The upstream library version includes several properties not supported by the DeepWork schema validator. Apply these fixes to `job.yml`:

1. **Remove `hidden: true`** from all steps — use `exposed: false` or omit (default is false)
2. **Replace `directory:` with `file:`** in output definitions — `directory` is not a valid output type
3. **Remove `description:` from outputs** — output objects only support `file` and `doc_spec`
4. **Fix inputs with `file:` + `description:`** — must be `{file, from_step}` not `{file, description}`
5. **Add explicit dependencies** — every `from_step` reference requires the step in `dependencies` (transitive deps are not inferred)
